...
def load_config(config_file_path: str):
    ...
    return namespace, pod, whitelist_pod_enable, whitelist_pods, splunk_user_group, splunk_container, sftp_user, sftp_password, dir_lookup, file_newer_than_min, max_processes, wait_to_start_secs, blacklist_pods
...
def get_splunk_ip():
    try:
        cmd = f"""kubectl get all | grep splunkfwd | awk 'FNR == 2 {{print $4}}' """
...
def get_splunk():
    ...
    try:
        # Get Splunk Pod- Name
        splunk_name_list = []
        cmd = f"""kubectl get pods | grep splunk | awk -F' ' '{{print $1}}' """
        out, error = subprocess_obj.execute_cmd(cmd)
        if out is not None:
            out = out.splitlines()
        else:
...
def get_uid_gid_from_splunk(splunk_list: List[str], splunk_container: str, splunk_user_group: str):
    """
    ...
    try:
        ...
        if splunk_list:
            cmd = f"""kubectl exec -it {splunk_list[0]} -c {splunk_container} -- getent passwd {splunk_user_group} | awk -F':' '{{print $3 "," $4}}'"""
            logger.info(f"{cmd}")
            out, error = subprocess_obj.execute_cmd(cmd)
            logger.info(f"{out}")
            out = str(out).split(",")
...
List[str]:
    try:
        pods_list = []
        cmd = f'kubectl get pods -n {namespace} | grep c{pod} | grep -v csdp.*c.* | grep -v upgrade'
...
def execute(namespace, pod_name: str, script_dir, sftp_user, sftp_password, splunk_ip, file_newer_than_min, output_dir,
            dir_lookup, uid, gid):
    print("In execute() ......")
    sdp_stat_obj = SDP_STAT.SDP_STAT(namespace, script_dir, sftp_user, sftp_password, splunk_ip, file_newer_than_min, output_dir,
                                     dir_lookup, uid, gid)
...
if __name__ == '__main__':
    try:
        ...
        NAMESPACE, POD, WHITELIST_ENABLED, WHITELIST_PODS_LIST, SPLUNK_GROUP_USER, SPLUNK_CONTAINER, SFTP_USER, SFTP_PASSWORD, DIR_LOOKUP, FILE_NEWER_THAN_MIN, MAX_PROCESSES, WAIT_TO_START, BLACKLIST_POD = load_config(
            CONFIG_FILE_PATH)
        SPLUNK_IP = get_splunk_ip()
        SPLUNK_NAME_LIST = get_splunk()
        UID, GID = get_uid_gid_from_splunk(SPLUNK_NAME_LIST, SPLUNK_CONTAINER, SPLUNK_GROUP_USER)
        if WAIT:
            wait_to_start(WAIT_TO_START)

        main(NAMESPACE, POD, SCRIPT_DIR, OUTPUT_DIR, ARCHIVE_DIR, LOG_DIR, WHITELIST_ENABLED, WHITELIST_PODS_LIST,
