...
END"""
    ...
    def set_message(self, data_source_hostname, data_source_ip, kafka_data_source_builder: KafkaDataSourceBuilder) :
        ...
        kafka_data_source_builder.set_message_field("kpi_source", f'{data_source_hostname} > KPI_SDP.py > {self.utcTimestamp}')
    ...
    def main(self, args_val):
        ...
        try:
            ...
            # REPLICATION_STATUS
            log_file = str(self.script_dir) + "/output/" + str(sdp) + "_replication_logfile.log"
            ...
            filename = str(filename).strip() if filename else None
            if filename is not None:
                cmd = f"""egrep 'FirstInterrogation|FinalReport' /tmp/{filename} | tail -90 | awk -F" " '{{s += $3 + $5}} END {{print s}}' """
                count, error = self.update_command(sdp, cmd)
                count = str(count).strip() if count else None
