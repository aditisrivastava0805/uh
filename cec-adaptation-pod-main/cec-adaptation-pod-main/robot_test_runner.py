#!/usr/bin/env python3
"""
Robot Framework test runner for the five modules: KAFKA_UAF, NFS_DISK_STATUS_CHECK, 
POD_FILE_COLLECTOR, POD_FILE_SENDER, and SDP_STAT.

This script provides a comprehensive test execution framework with the following features:
...
class RobotTestRunner:
    ...
    def validate_environment(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                raise subprocess.CalledProcessError(result.returncode, 'robot --version')
        except (FileNotFoundError, subprocess.CalledProcessError):
            print("❌ Robot Framework not found")
            print("   Install with: pip install robotframework")
            return False
              # Check required libraries
        required_libs = ['robotframework-requests', 'robotframework-jsonlibrary']
        for lib in required_libs:
            try:
                import_name = lib.replace('robotframework-', '').replace('-', '')
                result = subprocess.run([sys.executable, '-c', f'import {import_name}'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                if result.returncode == 0:
                    print(f"✅ {lib} found")
                else:
                    print(f"⚠️  {lib} not found (optional)")
            except Exception:
                print(f"⚠️  {lib} not found (optional)")
        ...
        for module in self.target_modules:
            ...
            # Check for test files (flexible naming)
    ...
    def run_module_tests(self, module, tags=None, exclude_tags=None, parallel=False):
        ...
        print(f"   Command: robot --outputdir {module_results_dir} ...")
          try:
            ...
            # For Python 3.6 compatibility, avoid timeout parameter
            ...
            print(f"⏰ {module} tests timed out (30 minutes)")
            return False
    ...
    def run_all_tests_sequential(self, tags=None, exclude_tags=None, modules=None):
        """Run tests for all modules sequentially."""
        test_modules = modules if modules else self.target_modules
        results = {}
        
        print(f"🚀 Starting sequential test execution for modules: {', '.join(test_modules)}")
        start_time = datetime.datetime.now()
    ...
    def run_all_tests_parallel(self, tags=None, exclude_tags=None, modules=None, max_workers=3):
        """Run tests for all modules in parallel."""
        test_modules = modules if modules else self.target_modules
        results = {}
        
        print(f"🚀 Starting parallel test execution for modules: {', '.join(test_modules)}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_module = {
                executor.submit(self.run_module_tests, module, tags, exclude_tags): module 
                for module in test_modules
            }
            ...
            for future in concurrent.futures.as_completed(future_to_module):
                ...
                try:
                    ...
                    results[module] = {
                        'success': success,
                        'duration': 'N/A (parallel)',
                        'start_time': start_time.isoformat(),
                        'end_time': datetime.datetime.now().isoformat()
                    }
                except Exception as e:
                    print(f"❌ {module} generated an exception: {e}")
                    results[module] = {
                        'success': False,
                        'duration': 'N/A (exception)',
                        'start_time': start_time.isoformat(),
                        'end_time': datetime.datetime.now().isoformat(),
                        'error': str(e)
                    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Robot Framework test runner for KAFKA_UAF, NFS_DISK_STATUS_CHECK, POD_FILE_COLLECTOR, POD_FILE_SENDER, and SDP_STAT modules",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python robot_test_runner.py                                    # Run all module tests
  python robot_test_runner.py --modules KAFKA_UAF SDP_STAT      # Run specific modules
  python robot_test_runner.py --tags smoke                      # Run only smoke tests
  python robot_test_runner.py --parallel --max-workers 2        # Run tests in parallel
  python robot_test_runner.py --validate-only                   # Only validate environment
        """
    )
    ...
    runner = RobotTestRunner(workspace_root)
